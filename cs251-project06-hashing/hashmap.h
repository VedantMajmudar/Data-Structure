...
// 
// hashmap implements a hash table assuming a hash function with probing;
// the class is designed to handle collision buy using a bool operater.
// The class provides the underlying hash table along with insert() and 
// search() functions the user of this class must provided a Hash(key, N) 
// function that hashes the given key and returns an integer in the range 0..N-1, 
...
// copy constructor:
//
//The given data base can be accessed by Hashmap.
//Where as the empty data base can be accessed by other.hashmap.
//
       hashmap(const hashmap& other)
       {
              //Declaring the New structure to store the data 
              N = other.N;
              HT = new HashEntry[N];
              //Copying the data for the orignal to the new one. 
  ...
  bool insert(KeyT key, ValueT value, function<int(KeyT,int)> Hash)
  {
    ...
    else  // valid array index, store into hash table:
    {
       ...
       if(HT[index].ContainsData == true)
       {      
              int pin = index;
              while(HT[index].ContainsData == true) //looking up next index.
              {
                     index++;//increaing index
                     if(index == N)
                            index = 0;
                     if(index == pin)
                            return false;
  ...
  bool search(KeyT key, ValueT& value, function<int(KeyT,int)> Hash) const
  {
    ...
    else if (HT[index].ContainsData)  // hash table contains data:
    {
       ...
       //Checking if the data related to the correct date.
       if(HT[index].Key != key)
       {
              int pin = index;
              while(HT[index].Key != key )
              {      
                     index++;
                     if(index == N)
                            index = 0;
                     if(index == pin)//If index increases more than the N..going to 0th index.
                            return false;
              }     
        }
