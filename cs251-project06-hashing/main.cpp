...
struct LotteryData
{
       string Date; //Adding a Date to verufy the date in  Linear probing.  
	string Numbers;
	string MegaBall;
	string Multiplier;
...
bool inputData(string filename, hashmap<string, LotteryData>& hmap)
{
	...
	while (getline(infile, line))
	{
		...
		// 
		getline(s, thedate, ',');  // first value => Date
		getline(s, numbers, ',');   // second value => Lotery Number
		getline(s, megaball, ',');   // third Value => Mega Number
		getline(s, multiplier, ',');  // fothe value =>Multiplier
...
int main()
{
	...
	string thedate;
       ...
       hashmap<string, LotteryData> Nhmap(hmap);
